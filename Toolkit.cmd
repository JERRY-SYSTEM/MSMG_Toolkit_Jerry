...
for %%i in (C_AADBrokerPlugin,C_AccountsControl,C_BioEnrollment,C_ClientCBS,C_CloudExperienceHost,C_Cortana,C_DesktopAppInstaller,C_EasyTransfer,C_EdgeChromium,C_EdgeWebView,C_GameExplorer,C_GamingApp,C_InputApp,C_InternetExplorer,C_KernelDebugging,C_ManualSetup,C_NETNativeFramework16,C_NETNativeFramework17,C_NETNativeFramework22,C_NETNativeRuntime16,C_NETNativeRuntime17,C_NETNativeRuntime22,C_OfflineFiles,C_PinEnrollment,C_PrintDialog,C_RemoteDesktopClient,C_RemoteDesktopServer,C_SearchApp,C_SecurityCenter,C_ShellExperienceHost,C_StartMenuExperienceHost,C_UIXaml20,C_UIXaml24,C_UIXaml27,C_UndockedDevKit,C_VCLibs140UWP,C_VCLibs140UWPDesktop,C_WindowsErrorReporting,C_WindowsFirewall,C_WindowsStore,C_WindowsStoreCore,C_WindowsUpdate,C_WinSAT,C_XboxIdentityProvider,C_XboxCore,C_XboxApp) do (
	if "%%i" equ "C_Cortana" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "18363" set "%%i=*"
	if "%%i" neq "C_Cortana" set "%%i=*"
)
...
if exist "%InstallEsd%" (
    ...
    :: 导出源映像为 WIM 映像
    "%Bin%\wimlib-imagex.exe" export "%InstallEsd%" %ImageIndexNo% "%InstallWim%" --compress=LZX --solid
...
if "%SelectedSourceOS%" neq "w7" if "%SelectedSourceOS%" neq "w81" (
	...
	echo.===============================================================================
	echo.                                 系统精简
	echo.     使用前请先看文档^(https://github.com/JERRY-SYSTEM/MSMG_Toolkit_Jerry^)
	echo.===============================================================================
	echo.  [1]   Internet
	echo.
	echo.  [2]   多 媒 体
	...
	echo.  [8]   兼容性
	echo.
	echo.  [9]   开始移除 Windows 组件
	echo.===============================================================================
	echo.
	choice /C:123456789 /N /M "请输入你的选项 ："
	if errorlevel 9 goto :RemoveWindowsComponents
	if errorlevel 8 goto :ComponentsCompatibilityMenu
...
if "%SelectedSourceOS%" neq "w7" if "%SelectedSourceOS%" neq "w81" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" (
	...
	if "%SelectedSourceOS%" equ "w11" (
		echo.  [1] %C_EdgeChromium% Edge Chromium
		echo.        基于 Chromium 的 Microsoft Edge web 浏览器。
		echo.        依赖于        ：Edge WebView
		echo.        兼容性        ：推荐
		echo.
	)
	if "%SelectedSourceOS%" equ "w10" (
		echo.  [2] %C_EdgeChromium% Edge Chromium
		echo.        基于 Chromium 的 Microsoft Edge web 浏览器。
		echo.        兼容性        ：推荐
		echo.
	)
	if "%SelectedSourceOS%" equ "w11" (
		echo.  [2] %C_EdgeWebView% Edge WebView
	...
	if /i "!MenuChoice!" neq "A" for %%# in (!MenuChoice!) do (
		...
		if "%%#" equ "1" if "%SelectedSourceOS%" equ "w11" if "%C_EdgeChromium%" neq "*" (
		...
		if "%%#" equ "2" if "%SelectedSourceOS%" equ "w10" if "%C_EdgeChromium%" neq "*" ( if "%C_EdgeChromium%" equ "+" ( set "C_EdgeChromium=-" ) else ( set "C_EdgeChromium=+" ) )
		if "%%#" equ "2" if "%SelectedSourceOS%" equ "w11" if "%C_EdgeWebView%" neq "*" (
		...
		if "%%#" equ "3" if "%C_InternetExplorer%" neq "*" ( if "%C_InternetExplorer%" equ "+" ( set "C_InternetExplorer=-" ) else ( set "C_InternetExplorer=+" ) )
...
if "%SelectedSourceOS%" neq "w7" if "%SelectedSourceOS%" neq "w81" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" (
	...
	echo.
	echo.  [ 9] %C_WindowsMediaPlayer% Windows Media Player
	...
	echo.
	echo.  [11] %C_WindowsThemes% Windows 主题
...
if "%SelectedSourceOS%" neq "w7" if "%SelectedSourceOS%" neq "w81" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" (
	...
	echo.         示）非常有用。
	echo.
	echo.  [ 2] %C_CEIP% 客户体验改善计划（CEIP）
	echo.         Windows 客户体验改善计划（CEIP）收集有关客户如何使用有关他们遇到的某
	...
	if /i "!MenuChoice!" neq "A" for %%# in (!MenuChoice!) do (
		...
		if "%%#" equ "4" if "%C_KernelDebugging%" neq "*" ( 
		...
		if "%%#" equ "10" if "%C_WindowsErrorReporting%" neq "*" ( 
...
if "%SelectedSourceOS%" neq "w7" if "%SelectedSourceOS%" neq "w81" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" (
	...
	if /i "!MenuChoice!" neq "A" for %%# in (!MenuChoice!) do (
		...
		if "%%#" equ "9" if "%C_ManualSetup%" neq "*" ( if "%C_ManualSetup%" equ "+" ( set "C_ManualSetup=-" ) else ( set "C_ManualSetup=+" ) )
		if "%%#" equ "10" (
			if "%C_Narrator%" equ "+" (
				set "C_Narrator=-"
				set "C_NarratorQuickStart=-"
		...
		if "%%#" equ "11" ( if "%C_Notepad%" equ "+" ( set "C_Notepad=-" ) else ( set "C_Notepad=+" ) )
		if "%%#" equ "12" (
		...
		if "%%#" equ "13" if "%SelectedSourceOS%" equ "w10" ( if "%C_Paint%" equ "+" ( set "C_Paint=-" ) else ( set "C_Paint=+" ) )
		if "%%#" equ "14" ( if "%C_ProjFS%" equ "+" ( set "C_ProjFS=-" ) else ( set "C_ProjFS=+" ) )
		if "%%#" equ "15" if "%C_SecurityCenter%" neq "*" (
		...
		if "%%#" equ "16" ( if "%C_StepsRecorder%" equ "+" ( set "C_StepsRecorder=-" ) else ( set "C_StepsRecorder=+" ) )
		if "%%#" equ "17" ( if "%C_StorageSpaces%" equ "+" ( set "C_StorageSpaces=-" ) else ( set "C_StorageSpaces=+" ) )
		if "%%#" equ "18" ( if "%C_SystemRestore%" equ "+" ( set "C_SystemRestore=-" ) else ( set "C_SystemRestore=+" ) )
		if "%%#" equ "19" ( if "%C_WindowsBackup%" equ "+" ( set "C_WindowsBackup=-" ) else ( set "C_WindowsBackup=+" ) )
		if "%%#" equ "20" if "%C_WindowsFirewall%" neq "*" ( if "%C_WindowsFirewall%" equ "+" ( set "C_WindowsFirewall=-" ) else ( set "C_WindowsFirewall=+" ) )
		if "%%#" equ "21" ( if "%C_WindowsSubsystemForLinux%" equ "+" ( set "C_WindowsSubsystemForLinux=-" ) else ( set "C_WindowsSubsystemForLinux=+" ) )
		if "%%#" equ "22" ( if "%C_WindowsToGo%" equ "+" ( set "C_WindowsToGo=-" ) else ( set "C_WindowsToGo=+" ) )
		if "%%#" equ "23" if "%C_WindowsUpdate%" neq "*" ( if "%C_WindowsUpdate%" equ "+" ( set "C_WindowsUpdate=-" ) else ( set "C_WindowsUpdate=+" ) )
		if "%%#" equ "24" ( if "%C_Wordpad%" equ "+" ( set "C_Wordpad=-" ) else ( set "C_Wordpad=+" ) )
		if /i "%%#" equ "X" goto :SelectWindowsComponentsMenu
	)
...
if "%SelectedSourceOS%" neq "w7" if "%SelectedSourceOS%" neq "w81" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" (
	...
	echo.  [ 4] %C_AsyncTextService% Async Text 服务（Microsoft.AsyncTextService）
	...
	echo.         图和屏幕录像。
	if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" echo.         依赖于        ：Windows 用户体验
	echo.
	...
	echo.
	echo.  [15] %C_FileExplorer% 文件资源管理器（Microsoft.Windows.FileExplorer）
	...
	echo.
	echo.  [18] %C_Edge% Microsoft Edge（Microsoft.MicrosoftEdge）
	echo.         Microsoft Edge 是 Windows 设备中存在的默认 Web 浏览器。
	echo.
	echo.  [19] %C_EdgeDevToolsClient% Microsoft Edge Dev 工具客户端（Microsoft.MicrosoftEdgeDevToolsClient）
	echo.         Microsoft Edge Dev 工具客户端是 Edge 浏览器的扩展，包含面向 Web 开发者的 Dev 工具。
	...
	echo.
	echo.  [21] %C_WindowsStoreCore% Microsoft Store Core
	echo.         Microsoft Store Core 是 Windows 应用商店应用正常运行所需的核心服务。
	echo.         兼容性        ：Microsoft Store
	echo.
	echo.  [22] %C_NarratorQuickStart% 讲述人（Microsoft.Windows.NarratorQuickStart）
	echo.         讲述人主页是一款现代讲述人应用，可大声朗读屏幕上的文本，使盲人或视力
	echo.         低下的人更容易使用他们的计算机。
	echo.         依赖于        ：讲述人
	echo.
	echo.  [23] %C_OOBENetworkConnectionFlow% Network Connection Flow（Microsoft.Windows.OOBENetworkConnectionFlow）
	...
	if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" (
		echo.  [24] %C_NcsiUwpApp% 网络连接状态指示器（NCSI）（NcsiUwpApp）
		echo.         网络连接状态指示器（NCSI）是一个系统应用，负责监视网络连接的状态并在
		echo.         任务栏中显示网络图标。它还会执行定期检查以验证你的网络连接是否正常工
		echo.         作。
		echo.
	)
	echo.  [25] %C_CloudExperienceHost% 全新安装体验（OOBE）（Microsoft.Windows.CloudExperienceHost）
	...
	echo.
	if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "18363" (
		echo.  [32] %C_InputApp% 外壳输入应用程序（InputApp）
		echo.         输入应用是系统应用，用于为备用用户输入文本输入处理器（TIP）和语言栏提供
		echo.         支持。在应用程序（笔和墨迹、语音等）中启用高级用户输入服务
		echo.         兼容性        ：Office、推荐、触摸屏设备
		echo.
	)
	if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" (
		echo.  [33] %C_UndockedDevKit% Shell Services ^(MicrosoftWindows.UndockedDevKit^)
	...
	echo.  [34] %C_SkypeORTC% Skype ORTC
	echo.         Skype ORTC 是一组 API，允许开发人员向其应用程序添加实时通信功能。它基于开放
	echo.         实时通信（ORTC）标准，此标准是实时通信的免版税开源规范。
	echo.
	echo.
	echo.  [35] %C_SmartScreen% SmartScreen（Microsoft.Windows.AppRep.ChxApp）
	echo.         Windows Defender SmartScreen 提供网络钓鱼和恶意软件筛选器，旨在通过针对包含
	echo.         已知威胁的网站黑名单扫描用户访问的 URL 来帮助防止攻击。
	echo.
	if "%ImageBuild%" geq "18362" if "%ImageBuild%" leq "22631" (
		echo.  [36] %C_StartMenuExperienceHost% “开始”菜单（Microsoft.Windows.StartMenuExperienceHost）
	...
	if "%ImageFlag%" neq "EnterpriseS" if "%ImageFlag%" neq "EnterpriseSN" (
		echo.  [37] %C_SecureAssessmentBrowser% 参加测试（Microsoft.Windows.SecureAssessmentBrowser）
		echo.         参加测试是一个基于 Web 的应用，允许用户对各种主题进行评估，例如安全意识、
		echo.         合规性和 IT 熟练程度。
		if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" echo.         依赖于        ：WebView2 Runtime
		echo.
	)
	if "%ImageBuild%" geq "19045" if "%ImageBuild%" leq "22631" (
		echo.  [38] %C_ClientCBS% Windows 用户体验（MicrosoftWindows.Client.CBS）
		...
		if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "19045" echo.          兼容性        ：Microsoft Store、推荐、触摸屏设备
	...
	echo.  [41] %C_CBSPreview% Windows Barcode Preview（Windows.CBSPreview）
	echo.         Windows Barcode Preview 应用是一个内置应用，可用于预览使用设备相机扫描
	echo.         的条形码。
	echo.
	echo.  [42] %C_LockApp% Windows 默认锁屏（Microsoft.LockApp）
	echo.         Windows 默认锁屏界面是首次打开计算机或将其从睡眠状态唤醒时看到的屏幕。
	echo.         锁定计算机时也可以显示它。
	echo.
	if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "19045" echo.  [43] %C_WindowsDefender% Windows Defender + Windows 安全中心（Microsoft.Windows.SecHealthUI）
	if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" echo.  [43] %C_WindowsDefender% Windows Defender + Windows 安全中心（Microsoft.SecHealthUI）
	...
	if "%ImageBuild%" geq "22621" if "%ImageBuild%" leq "22631" echo.         依赖于        ：Microsoft UI Xaml 2.4、Microsoft Visual C++ 2015 UWP Runtime
	echo.
	echo.  [44] %C_BioEnrollment% Windows Hello 安装程序（Microsoft.BioEnrollment）
	...
	echo.  [49] %C_PeopleExperienceHost% Windows Shell Experience（Microsoft.Windows.PeopleExperienceHost）
	echo.         Windows 命令行管理程序体验是用于管理“人脉”应用的系统应用。它还为 
	echo.         Windows 中的其他功能（如“开始”菜单中的“人脉栏”和“人物磁贴”）提供底层基础
	echo.         结构。
	echo.
	echo.  [50] %C_AADBrokerPlugin% 工作或学校帐户（Microsoft.AAD.BrokerPlugin）
	...
	echo.         兼容性        ：Microsoft Store
	echo.
	echo.  [51] %C_XboxCore% Xbox Core
	echo.         Xbox Core 是 Xbox 应用运行所需的核心服务。
	echo.         兼容性        ：Xbox
	echo.
	echo.  [52] %C_XboxGameCallableUI% Xbox Game Callable 用户界面（Microsoft.XboxGameCallableUI）
	...
	if /i "!MenuChoice!" neq "A" for %%# in (!MenuChoice!) do (
		...
		if "%%#" equ "21" if "%C_WindowsStoreCore%" neq "*" (
			if "%C_WindowsStoreCore%" equ "+" (
				if "%SelectedSourceOS%" equ "w10" set "C_ServicesStoreEngagement=-"
				if "%ImageBuild%" equ "17763" set "C_SolitaireCollection=-"
				if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" set "C_StickyNotes=-"
		...
		if "%%#" equ "24" if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" ( if "%C_NcsiUwpApp%" equ "+" ( set "C_NcsiUwpApp=-" ) else ( set "C_NcsiUwpApp=+" ) )
		if "%%#" equ "25" if "%C_CloudExperienceHost%" neq "*" (
		...
		if "%%#" equ "26" ( if "%C_PinningConfirmationDialog%" equ "+" ( set "C_PinningConfirmationDialog=-" ) else ( set "C_PinningConfirmationDialog=+" ) )
		if "%%#" equ "27" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22000" ( if "%C_QuickAssist%" equ "+" ( set "C_QuickAssist=-" ) else ( set "C_QuickAssist=+" ) )
		if "%%#" equ "28" ( if "%C_RetailDemoContent%" equ "+" ( set "C_RetailDemoContent=-" ) else ( set "C_RetailDemoContent=+" ) )
		if "%%#" equ "29" ( if "%C_XGpuEjectDialog%" equ "+" ( set "C_XGpuEjectDialog=-" ) else ( set "C_XGpuEjectDialog=+" ) )
		if "%%#" equ "30" ( if "%C_SettingSync%" equ "+" ( set "C_SettingSync=-" ) else ( set "C_SettingSync=+" ) )
		if "%%#" equ "31" if "%C_ShellExperienceHost%" neq "*" (
			if "%C_ShellExperienceHost%" equ "+" ( 
				set "C_ShellExperienceHost=-"
				if "%ImageBuild%" geq "18362" if "%ImageBuild%" leq "22000" if "%C_StartMenuExperienceHost%" neq "*" set "C_StartMenuExperienceHost=-"
			) else ( 
				set "C_ShellExperienceHost=+"
			)
		)
		if "%%#" equ "32" if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "18363" if "%C_InputApp%" neq "*" ( if "%C_InputApp%" equ "+" ( set "C_InputApp=-" ) else ( set "C_InputApp=+" ) )
		rem if "%%#" equ "33" if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" if "%C_UndockedDevKit%" neq "*" ( if "%C_UndockedDevKit%" equ "+" ( set "C_UndockedDevKit=-" ) else ( set "C_UndockedDevKit=+" ) )
		if "%%#" equ "34" ( if "%C_SkypeORTC%" equ "+" ( set "C_SkypeORTC=-" ) else ( set "C_SkypeORTC=+" ) )
		if "%%#" equ "35" ( if "%C_SmartScreen%" equ "+" ( set "C_SmartScreen=-" ) else ( set "C_SmartScreen=+" ) )
		if "%%#" equ "36" if "%ImageBuild%" geq "18362" if "%ImageBuild%" leq "22631" if "%C_StartMenuExperienceHost%" neq "*" (
			if "%C_StartMenuExperienceHost%" equ "+" ( 
				set "C_StartMenuExperienceHost=-"
			) else ( 
				if "%ImageBuild%" geq "18362" if "%ImageBuild%" leq "22000" if "%C_ShellExperienceHost%" neq "*" set "C_ShellExperienceHost=+"
				set "C_StartMenuExperienceHost=+"
			)
		)
		if "%%#" equ "37" if "%ImageFlag%" neq "EnterpriseS" if "%ImageFlag%" neq "EnterpriseSN" ( 
			if "%C_SecureAssessmentBrowser%" equ "+" ( 
				set "C_SecureAssessmentBrowser=-" 
			) else ( 
				set "C_SecureAssessmentBrowser=+" 
				if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" set "C_WebView2SDK=+"
			)
		)
		if "%%#" equ "38" if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" (
			if "%C_ClientCBS%" equ "+" (
				set "C_ClientCBS=-"
			) else (
				set "C_ClientCBS=+"
		...
		if "%%#" equ "39" ( if "%C_WebcamExperience%" equ "+" ( set "C_WebcamExperience=-" ) else ( set "C_WebcamExperience=+" ) )
		if "%%#" equ "40" if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" (
		...
		if "%%#" equ "41" ( if "%C_CBSPreview%" equ "+" ( set "C_CBSPreview=-" ) else ( set "C_CBSPreview=+" ) )
		if "%%#" equ "42" ( if "%C_LockApp%" equ "+" ( set "C_LockApp=-" ) else ( set "C_LockApp=+" ) )
		if "%%#" equ "43" (
		...
		if "%%#" equ "44" if "%C_BioEnrollment%" neq "*" ( if "%C_BioEnrollment%" equ "+" ( set "C_BioEnrollment=-" ) else ( set "C_BioEnrollment=+" ) )
		if "%%#" equ "45" (
		...
		if "%%#" equ "46" if "%C_PrintDialog%" neq "*" ( if "%C_PrintDialog%" equ "+" ( set "C_PrintDialog=-" ) else ( set "C_PrintDialog=+" ) )
		if "%%#" equ "47" (
		...
		if "%%#" equ "48" if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22000" ( if "%C_SearchApp%" equ "+" ( set "C_SearchApp=-" ) else ( set "C_SearchApp=+" ) )
		if "%%#" equ "49" (
		...
		if "%%#" equ "50" if "%C_AADBrokerPlugin%" neq "*" ( if "%C_AADBrokerPlugin%" equ "+" ( set "C_AADBrokerPlugin=-" ) else ( set "C_AADBrokerPlugin=+" ) )
		if "%%#" equ "51" if "%C_XboxCore%" neq "*" (
			if "%C_XboxCore%" equ "+" (
				if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" if "%C_GamingApp%" neq "*" set "C_GamingApp=-"
				if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "19045" if "%C_XboxApp%" neq "*" set "C_XboxApp=-"
				set "C_XboxCore=-"
	...
	if /i "!MenuChoice!" equ "A" (
		...
		set "C_SolitaireCollection=-"
		if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "22631" set "C_StickyNotes=-"
		if "%ImageBuild%" geq "18362" if "%ImageBuild%" leq "22631" if "%C_StartMenuExperienceHost%" neq "*" set "C_StartMenuExperienceHost=-"
		set "C_StorePurchaseApp=-"
		rem if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "22631" if "%C_UndockedDevKit%" neq "*" set "C_UndockedDevKit=-"
		...
		set "C_WindowsReaderPDF=-"
		if "%C_WindowsStore%" neq "*" set "C_WindowsStore=-"
		if "%C_WindowsStoreCore%" neq "*" set "C_WindowsStoreCore=-"
		if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "19045" if "%C_XboxApp%" neq "*" set "C_XboxApp=-"
...
if /i "!MenuChoice!" neq "A" if /i "!MenuChoice!" neq "D" for %%# in (!MenuChoice!) do (
	...
	if "%%#" equ "7" (
		if "%CC_MicrosoftStore%" equ "+" (
        		...
        		if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "19045" if "%CC_Recommended%" equ "-" if "%CC_TouchScreenDevices%" equ "-" set "C_ClientCBS=+"
        		if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" if "%CC_OOBE%" equ "-" if "%CC_Recommended%" equ "-" if "%CC_ShellSearch%" equ "-" if "%CC_TouchScreenDevices%" equ "-" set "C_ClientCBS=+"
	...
	if "%%#" equ "9" (
		if "%CC_OOBE%" equ "+" (
        		set "CC_OOBE=-"
        		if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" if "%CC_MicrosoftStore%" equ "-" if "%CC_Recommended%" equ "-" if "%CC_ShellSearch%" equ "-" if "%CC_TouchScreenDevices%" equ "-" set "C_ClientCBS=+"
	...
	if "%%#" equ "11" (
		if "%CC_Recommended%" equ "+" (
        		set "CC_Recommended=-"
        		if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "19045" if "%CC_MicrosoftStore%" equ "-" if "%CC_TouchScreenDevices%" equ "-" set "C_ClientCBS=+"
        		if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" if "%CC_MicrosoftStore%" equ "-" if "%CC_OOBE%" equ "-" if "%CC_ShellSearch%" equ "-" if "%CC_TouchScreenDevices%" equ "-" set "C_ClientCBS=+"
        		if "%CC_OOBE%" equ "-" if "%CC_MicrosoftStore%" equ "-" if "%CC_ShellSearch%" equ "-" set "C_CloudExperienceHost=+"
	        	set "C_EdgeChromium=+"
        		...
        		if "%ImageBuild%" equ "17763" if "%CC_ShellSearch%" equ "-" set "C_ShellExperienceHost=+"
        		if "%ImageBuild%" geq "18362" if "%ImageBuild%" leq "22631" set "C_ShellExperienceHost=+"
	...
	if "%%#" equ "12" (
		if "%CC_ShellSearch%" equ "+" (
        		set "CC_ShellSearch=-"
        		if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" if "%CC_MicrosoftStore%" equ "-" if "%CC_OOBE%" equ "-" if "%CC_Recommended%" equ "-" if "%CC_TouchScreenDevices%" equ "-" set "C_ClientCBS=+"
        		if "%ImageBuild%" geq "17763" if "%ImageBuild%" leq "18363" (
	...
	if "%%#" equ "13" (
		if "%CC_TouchScreenDevices%" equ "+" (
        		...
        		if "%ImageBuild%" geq "19041" if "%ImageBuild%" leq "19045" if "%CC_MicrosoftStore%" equ "-" if "%CC_Recommended%" equ "-" set "C_ClientCBS=+"
        		if "%ImageBuild%" geq "22000" if "%ImageBuild%" leq "22631" if "%CC_MicrosoftStore%" equ "-" if "%CC_OOBE%" equ "-" if "%CC_Recommended%" equ "-" if "%CC_ShellSearch%" equ "-" set "C_ClientCBS=+"
...
:: 设置组件状态标识
for %%i in (C_AdobeFlashForWindows,C_EdgeChromium,C_EdgeWebView,C_InternetExplorer,C_FirstLogonAnimation,C_GameExplorer,C_LockScreenBackground,C_ScreenSavers,C_SnippingTool,C_SoundThemes,C_SpeechRecognition,C_Wallpapers,C_WindowsMediaPlayer,C_WindowsPhotoViewer,C_WindowsThemes,C_WindowsTIFFIFilter,C_WinSAT,C_OfflineFiles,C_OpenSSH,C_RemoteDesktopClient,C_RemoteDifferentialCompression,C_SimpleTCPIPServices,C_TelnetClient,C_TFTPClient,C_WalletService,C_WindowsMail,C_AssignedAccess,C_CEIP,C_FaceRecognition,C_KernelDebugging,C_LocationService,C_PicturePassword,C_PinEnrollment,C_UnifiedTelemetryClient,C_WiFiNetworkManager,C_WindowsErrorReporting,C_WindowsInsiderHub,C_HomeGroup,C_MultiPointConnector,C_OneDrive,C_RemoteAssistance,C_RemoteDesktopServer,C_RemoteRegistry,C_WorkFoldersClient,C_AccessibilityTools,C_Calculator,C_DeviceLockdown,C_EaseOfAccessCursors,C_EaseOfAccessThemes,C_EasyTransfer,C_FileHistory,C_Magnifier,C_Narrator,C_Notepad,C_OnScreenKeyboard,C_Paint,C_ProjFS,C_SecurityCenter,C_StepsRecorder,C_StorageSpaces,C_SystemRestore,C_WindowsBackup,C_WindowsFirewall,C_WindowsSubsystemForLinux,C_WindowsToGo,C_WindowsUpdate,C_Wordpad,C_AADBrokerPlugin,C_AccountsControl,C_AddSuggestedFoldersToLibraryDialog,C_AppResolverUX,C_AssignedAccessLockApp,C_AsyncTextService,C_BioEnrollment,C_CallingShellApp,C_CapturePicker,C_CBSPreview,C_ClientCBS,C_CloudExperienceHost,C_ContentDeliveryManager,C_Cortana,C_CredDialogHost,C_ECApp,C_Edge,C_EdgeDevToolsClient,C_FileExplorer,C_FilePicker,C_InputApp,C_LockApp,C_MapControl,C_NarratorQuickStart,C_NcsiUwpApp,C_OOBENetworkCaptivePortal,C_OOBENetworkConnectionFlow,C_ParentalControls,C_PeopleExperienceHost,C_PinningConfirmationDialog,C_PrintDialog,C_PPIProjection,C_QuickAssist,C_RetailDemoContent,C_SearchApp,C_SecureAssessmentBrowser,C_SettingSync,C_ShellExperienceHost,C_SkypeORTC,C_SmartScreen,C_StartMenuExperienceHost,C_UndockedDevKit,C_WebcamExperience,C_WebView2SDK,C_Win32WebViewHost,C_WindowsDefender,C_WindowsMixedReality,C_WindowsReaderPDF,C_WindowsStoreCore,C_XboxCore,C_XboxGameCallableUI,C_XGpuEjectDialog) do (
	if "!%%i!" equ "-" (
...
if "%TrimEditions%" equ "Yes" (
	"%Bin%\wimlib-imagex.exe" export "%InstallWim%" %ImageIndexNo% "%Temp%\rebuild.wim" --compress=LZX
